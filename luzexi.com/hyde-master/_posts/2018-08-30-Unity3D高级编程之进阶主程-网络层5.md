...
整个map就是以这种“类型|数据”的方式存储，存储的方式也是顺序，不需要排序，不需要解析符号和类型，类型直接用byte来表示，能用byte存储绝不用字符串形式存储，能减少byte使用个数的尽量减少byte的使用个数，能合并的尽量合并为一个byte。

因此比JSON少了解析，压缩了数据的大小，使得MessagePack更快更小，就像它自己所说的那样 ‘It’s like JSON. but fast and small.’。
