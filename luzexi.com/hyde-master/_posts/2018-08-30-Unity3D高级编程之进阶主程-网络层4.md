...
###### 解决方案3：合并请求，并逐个发送合并后的请求包。

为了提高网络效率，客户端更希望的是多次请求能够被同时发出，收到的响应顺序也要按原来请求的顺序响应，并且要可以自定义回调句柄满足多样化的功能扩展。

同时发起多次连接会导致响应的顺序不同，但合并多个请求进一个请求中，发送这个合并包则可以做到与发起多个连接同样的效果，并且响应也可以是一致的。

所以我主张只开一个线程处理请求，用排队的方式处理请求和响应。
