...
当接受数据的顺序无法确定时，如果还用顺序接受数据的方式处理出具则有风险，比如我们在发送，军团，任务，角色数据请求后，希望能够在三者都到齐的情况下执行某个程序逻辑，如果得到的数据有顺序，则我可以知道在收到角色数据后表明，所有的数据都到齐了，于是我们可以执行逻辑程序了。

那么怎么在没有数据顺序的情况下处理，处理不同连接请求数据的合并逻辑呢。

多次请求数据的合并处理问题，其本质问题是‘如何判断我们需要的多条数据已经到达’。

解决方案1：等待所有数据到位后调用逻辑处理。

这种方式最常见，也最普遍的做法，即等待并辨认多次请求是否全数收到。

用伪代码来表示如下：

        void StartRequest( request_list )
        {
