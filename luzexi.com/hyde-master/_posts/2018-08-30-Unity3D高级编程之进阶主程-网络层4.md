...
解决方案1：等待所有数据到位后调用程序执行逻辑处理。

那么怎么在没有数据顺序的情况下处理不同连接请求数据的合并逻辑呢。

假设这种多次请求之间不需要有逻辑顺序的HTTP请求，则可以使用多次同时发起请求，并且等待所有请求结束后再做逻辑处理。

多次请求数据的合并处理问题，其本质问题是‘如何判断我们需要的多条数据已经到达’。

为了能更快的，更高效的得到HTTP请求数据，在同一时间同时向服务器发起多个HTTP请求，并且等待所有请求都得到响应后才执行逻辑程序，即等待并辨认多次请求是否全数收到。

在Unity3D上伪代码如下，重点在注释：
...
这种请求方式会同时开启多个HTTP连接来想服务器请求数据，极大限度的加速了数据的回调速度，最大限度的加速了服务器端的CPU利用率，但请求彼此之间是没有顺序关系的，因此服务器端的执行顺序也是无法保障的。

服务器端执行顺序无法保障则容易出现逻辑顺序问题，比如会有多个这样的请求，购买物品，出售物品，使用物品，三个请求一起被发起，服务器接受到的请求顺序有可能是，使用物品，出售物品，再购买物品，这样的话就存在逻辑问题，导致跟我们原本的设想存在偏差，物品可能先被使用或出售，而不是被先购买，导致可能没有物品可被购买或出售，跟我们的预期不一致。

虽然连接请求速度更快了，却无法保证有预期的顺序，在需要顺序的逻辑上会出现诸多问题。

虽然在需要逻辑顺序的连接请求上无法运作，但在不需要逻辑顺的连接请求上却是个极好用的方案。

解决方案2：逐个发起请求，保证顺序

这种方式最常见也比较普遍的做法，在网页端，安卓原生端，苹果原生端都使用这种方式来集合数据，即每次只处理一个请求，当这个请求结束后再处理下一个请求，即一个请求对应一个功能模块，结束当前功能模块后再发起下一个请求。

解决方案2：合并请求。

客户端更希望是，在多次请求发出后，收到的响应也是按原来请求的顺序响应的。所以我主张只开一个线程处理请求，用排队的方式处理请求和响应。
